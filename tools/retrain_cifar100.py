...
def best_acc_weights(weights_folder):
    ...
    best_files = sorted(best_files,
                        key=lambda w: int(re.search(regex_str, w).groups()[1]))
    return best_files[-1]
...
def eval_training(epoch=0, tb=True):
    ...
    print(
        ...
        .format(epoch, test_loss / len(cifar100_test_loader.dataset),
...
def get_test_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True):
    ...
    cifar100_test = torchvision.datasets.CIFAR100(root='/data/public/cifar',
                                                  train=False,
                                                  download=True,
...
if __name__ == '__main__':
    ...
    net = Network(args.model,
                  100,
                  eval('genotypes.%s' % args.arch),
                  dropout=0.).cuda()
    ...
    if args.resume:
        recent_folder = most_recent_folder(os.path.join(
            './exps/retrain_cifar100', args.model),
                                           fmt='%A_%d_%B_%Y_%Hh_%Mm_%Ss')
        if not recent_folder:
            raise Exception('no recent folder were found')

        checkpoint_path = os.path.join('./exps/retrain_cifar100', args.model,
                                       recent_folder)
    ...
    if args.resume:
        best_weights = best_acc_weights(
            os.path.join('./exps/retrain_cifar100', args.model, recent_folder))
        if best_weights:
            weights_path = os.path.join('./exps/retrain_cifar100', args.model,
    ...
    for epoch in range(1, 200 + 1):
        ...
        if epoch > 120 and best_acc < acc:
            weights_path = checkpoint_path.format(net=args.model,
                                                  epoch=epoch,
                                                  type='best')
